...
shaka.util.StreamUtils = class {
   ...
   * @param {?shaka.extern.Variant=} activeVariant
   */
  static chooseCodecsAndFilterManifest(manifest, preferredVideoCodecs,
    ...
    manifest.variants = manifest.variants.filter((variant) => {
      ...
      if (activeVariant) {
        const codecs = shaka.util.StreamUtils.compareVariantCodecs_(
            activeVariant, variant);
        if (codecs.matchingAudio && codecs.matchingVideo) {
          return true;
        }
      }
      if (audio) {
  ...
  static async filterManifestByMediaCapabilities(
    ...
    manifest.variants = manifest.variants.filter((variant) => {
      const supported = StreamUtils.checkVariantSupported_(
          variant, keySystem, keySystemsMapping, activeVariant);
   ...
   * @return {{matchingAudio: boolean, matchingVideo: boolean}}
   * @private
   */
  static compareVariantCodecs_(a, b) {
    const aVideoCodec = a.video && a.video.codecs;
    const aAudioCodec = a.audio && a.audio.codecs;

    const bAudioCodec = b.audio && b.audio.codecs;
    const bVideoCodec = b.video && b.video.codecs;

    return {
      matchingAudio: aAudioCodec === bAudioCodec,
      matchingVideo: aVideoCodec === bVideoCodec,
    };
  }
   ...
   * @private
   */
  static checkVariantSupported_(
      variant, keySystem, keySystemsMapping, activeVariant = undefined) {
    let sameCodecsAsActiveVariant = false;
    if (activeVariant) {
      const codecs =
          shaka.util.StreamUtils.compareVariantCodecs_(activeVariant, variant);
      sameCodecsAsActiveVariant = codecs.matchingAudio && codecs.matchingVideo;
    }

    if (!sameCodecsAsActiveVariant) {
  ...
  static getStreamSummaryString_(stream) {
    ...
    if (shaka.util.StreamUtils.isVideo(stream)) {
      return 'type=video' +
             ...
             ' videoDrmInfos=' + (stream.drmInfos ? stream.drmInfos.length : 0);
    }
